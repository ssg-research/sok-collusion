...
Key public API:
    build_oracle_pool_image()    - oracle-recon pool for image tasks (validation source)
    build_oracle_pool_tabular()  - oracle-recon pool for tabular tasks (validation source)
    build_hybrid_overlap_image() - legacy Fredrikson + oracle padding (kept for back-compat)
    build_hybrid_overlap_tabular() - legacy Fredrikson + oracle padding (kept for back-compat)
...
# ── Oracle-source overlap pools (validation stage) ──────────────────────────
#
# "Oracle reconstruction" simulates the upper bound of a perfect data
# reconstruction attack by sampling p% of the adversary's auxiliary-data input
# directly from D_train (the records a perfect reconstruction would recover).
# (1-p)% is sampled from D_out - a distribution-matched proxy that represents
# the standard published baseline assumption (e.g., LiRA's shadow data).
#
# These pools are used in Stage 2 validation. Stage 5 production swaps the
# oracle source for cached records recovered by Geiping (NeurIPS 2020).

def build_oracle_pool_image(
    d_train: Dataset,
    d_out: Dataset,
    p: float,
    n_a: int,
    ...
    """
    Build adversary auxiliary pool for image tasks (MIA / DIA) with oracle recon.

    Pool of size n_a:
        n_in  = p · n_a   - oracle samples from d_train (perfect-recon proxy)
        n_out = (1-p) · n_a - distribution-matched samples from d_out

    `reserved_train_idx` / `reserved_out_idx` (optional): indices within d_train /
    d_out that MUST NOT be sampled into the pool. Used by MIA to reserve the
    challenge set so pool and challenge records are physically disjoint
    (preserves Phase A0 LiRA disjointness fix).

    Returns (pool, pool_in_indices, composition_log) where pool_in_indices are
    the positions within the pool of records that come from D_train (n_in records).
    """
    rng = np.random.RandomState(seed)
    n_in = int(p * n_a)
    n_out = n_a - n_in
        ...
        if reserved_train_idx is not None and len(reserved_train_idx) > 0:
            ...
            if len(available) < n_in:
                raise ValueError(
                    ...
                    f"need {n_in}, have {len(available)} "
                    f"(train_size={train_size}, reserved={len(reserved_train_idx)})"
                )
            in_idx = rng.choice(available, size=n_in, replace=False)
        else:
            in_idx = rng.choice(train_size, size=n_in, replace=False)
        parts.append(Subset(d_train, in_idx.tolist()))
    ...
    if n_out > 0:
        ...
        if reserved_out_idx is not None and len(reserved_out_idx) > 0:
            ...
            out_idx = rng.choice(available, size=n_out, replace=False)
        else:
    ...
    composition_log = {
        "p": p, "n_a": n_a, "n_in": n_in, "n_out": n_out,
        "seed": seed, "in_idx": in_idx, "out_idx": out_idx,
        # Legacy keys for CSV row compatibility:
        "n_recon": 0, "n_oracle": n_in,
    }
    return pool, pool_in_indices, composition_log


def build_oracle_pool_tabular(
    x_train: np.ndarray,
    z_train: np.ndarray,
    x_out: np.ndarray,
    z_out: np.ndarray,
    ...
    """
    Build adversary auxiliary pool for tabular tasks with oracle recon.

    Returns (x_pool, z_pool, y_pool_or_None, composition_log).
    Tracks `pool_in_indices` (positions of in-records within pool) implicitly:
    the first n_in rows of (x_pool, z_pool, y_pool) are the in-records.

    z_pool is always 2-D (n_a, num_sensitive_attrs).
    y_pool is None unless y_train and y_out are provided.
    """
    rng = np.random.RandomState(seed)
    n_in = int(p * n_a)
    n_out = n_a - n_in
    ...
    z_train_2d = z_train if z_train.ndim == 2 else z_train[:, None]
    z_out_2d = z_out if z_out.ndim == 2 else z_out[:, None]

    if n_in > 0:
        in_idx = rng.choice(len(x_train), size=n_in, replace=False)
        x_parts.append(x_train[in_idx])
        z_parts.append(z_train_2d[in_idx])
        if y_train is not None:
            y_parts.append(y_train[in_idx])

    if n_out > 0:
        out_idx = rng.choice(len(x_out), size=min(n_out, len(x_out)), replace=False)
        x_parts.append(x_out[out_idx])
        z_parts.append(z_out_2d[out_idx])
        if y_out is not None:
            y_parts.append(y_out[out_idx])

    x_pool = np.concatenate(x_parts, axis=0) if x_parts else np.zeros((0, x_train.shape[1]))
    z_pool = np.concatenate(z_parts, axis=0) if z_parts else np.zeros((0, z_train_2d.shape[1]), dtype=int)
    y_pool = np.concatenate(y_parts, axis=0) if y_parts else None

    composition_log = {
        "p": p, "n_a": n_a, "n_in": n_in, "n_out": n_out, "seed": seed,
        "n_recon": 0, "n_oracle": n_in,
    }
    return x_pool, z_pool, y_pool, composition_log
...
# ── Tabular hybrid overlap pool (AIA) ────────────────────────────────────────

def build_hybrid_overlap_tabular(
    target_model: nn.Module,
    x_train: np.ndarray,
    z_train: np.ndarray,
    x_out: np.ndarray,
    ...
    """
    Build adversary auxiliary pool for tabular tasks (AIA).

    Returns (x_adv, z_adv, composition_log) as numpy arrays.

    Fredrikson reconstructions for tabular models produce flat tensors of shape
    (num_features,) which are used directly as feature vectors.
    Sensitive attributes z for reconstructed records are imputed as the majority
    class of z_train for the corresponding output class.
    """
    rng = np.random.RandomState(seed)
    ...
    # ── Fredrikson reconstructions ──────────────────────────────────────────
    if n_recon > 0:
        # For tabular inputs the model takes flat feature vectors
        recon_tensors = run_fredrikson_recon(
            target_model, input_size, num_classes, device,
            cache_dir, model_path,
        )
        recon_x = np.stack([recon_tensors[c].numpy() for c in range(n_recon)])
        # Impute z as majority value in z_train per column
        if z_train.ndim == 1:
            z_train_2d = z_train[:, None]
        else:
            z_train_2d = z_train
        majority_z = np.array([
            np.bincount(z_train_2d[:, col].astype(int)).argmax()
            for col in range(z_train_2d.shape[1])
        ])
        recon_z = np.tile(majority_z, (n_recon, 1))
        x_parts.append(recon_x)
        z_parts.append(recon_z)

    # ── Oracle overlap padding ──────────────────────────────────────────────
    if n_oracle > 0:
        oracle_idx = rng.choice(len(x_train), size=n_oracle, replace=False)
        x_parts.append(x_train[oracle_idx])
        z_parts.append(
            z_train[oracle_idx] if z_train.ndim == 1
            else z_train[oracle_idx]
        )

    # ── Disjoint out-of-training pool ───────────────────────────────────────
    if n_out > 0:
        out_idx = rng.choice(len(x_out), size=min(n_out, len(x_out)), replace=False)
        x_parts.append(x_out[out_idx])
        z_parts.append(
            z_out[out_idx] if z_out.ndim == 1
            else z_out[out_idx]
        )

    x_adv = np.concatenate(x_parts, axis=0)
    z_raw = np.concatenate(z_parts, axis=0)
    # Ensure z is always 2-D (num_samples, num_sensitive_attrs)
    z_adv = z_raw if z_raw.ndim == 2 else z_raw[:, None]
    ...
    return x_adv, z_adv, composition_log
